...
- 持久化快照容量上限由 256 MiB 提高到 512 MiB（对正常重任务最大 127 MB 快照留 4 倍余量；恢复内存峰值仍在 2 GiB worker 限额内）。
- 用户输入文件（`/workspace/input`）不再计入每轮工作区快照：输入为平台投放的不可变文件，崩溃恢复/沙箱切换/依赖升级换箱时改为从对象存储重新投放，多附件重任务的快照体积与每轮打包耗时永久下降。

### Fixed

- 快照因超出容量上限（或 Docker 传输短暂不可用）而失败时不再杀死任务：降级为跳过本次及后续快照并记录"已跳过工作区快照"中性事件，任务继续执行、最终产物校验不受影响；路径越界/校验和不符等完整性错误仍然致命。
- 快照辅助容器清理超时由 60s 提高到 180s 并自动重试一次，清理失败不再掩盖真正的导出错误（此前 ReadTimeout 曾掩盖"快照超容量"根因）。
