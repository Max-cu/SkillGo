...
- `AgentRun` 保存执行尝试、Worker 租约、心跳、重试次数和最终摘要；短期详细事件按保留策略清理，任务结论不会随文件到期而消失。
...
- 对话文字、任务摘要和审计记录持续保留；对话附件、任务输入与生成产物默认保留 15 天，重要任务可在工作台标记为“长期保留”。
- 管理员可在“存储管理”查看平台文件占用并立即执行到期清理；数据库无引用的孤儿文件经过 24 小时缓冲后回收。
