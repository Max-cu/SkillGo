...
删除会话或 Skill 会同步清理对应的托管文件；清空消息会保留文件。相关限制可通过 `SKILLGO_WORKSPACE_*` 环境变量调整。
...
运行摘要、用户消息和最终产物不会被后台留存任务删除。详细运行事件默认在成功后保留 7 天、失败或取消后保留 30 天，之后只保留任务输入、状态、产物和最终结果等摘要事件。清理周期和期限可通过 `SKILLGO_AGENT_RUN_CLEANUP_INTERVAL_SECONDS`、`SKILLGO_AGENT_RUN_SUCCESS_DETAIL_DAYS`、`SKILLGO_AGENT_RUN_FAILURE_DETAIL_DAYS` 调整。
